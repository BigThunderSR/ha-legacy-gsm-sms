...
**Why use this add-on?** The official Home Assistant SMS integration was removed in 2025.12, and the [HACS integration](../README.md#hacs-integration-deprecated) is now **deprecated and broken** since HA 2026.3.0 (the pre-built `python-gammu` wheels are no longer available). This add-on bundles Gammu and all required dependencies, making it the recommended solution for all Home Assistant installation types.
