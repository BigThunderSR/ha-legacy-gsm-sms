...
- **Terminate Existing Connection Before Reinit** 🔌
  ...
  - Properly releases the serial port for the new connection

## [2.18.5-test] - 2026-02-02

### Fixed

- **Modem Reinit After Call Ends** 🔄
  - SIM7600G-H modem gets into a hung state after handling incoming calls
  - Now performs a full Gammu reconnect AFTER cooldown completes
  - Uses existing `_reconnect_gammu()` with proper locking and connection validation
  - Safer than immediate soft reset: waits for cooldown, tests connection before continuing
...
- **Balance sensor values are now numeric** - If you have automations or templates using `sensor.sms_gateway_account_balance` or `sensor.sms_gateway_data_remaining`, you may need to update them:
  - `account_balance` changed from `"$3.00"` to `3.0` (numeric)
  - `data_remaining` changed from `"200.00 MB"` to `200.0` (numeric in MB)
  - Units are now displayed via `unit_of_measurement` attribute
...
- **All sensors now have proper device/state classes** - Enables better graphs, statistics, and energy dashboard integration
- **Added `unit_of_measurement`** to SMS count sensors ("messages")
- **Migration logic** for existing saved balance data to new numeric format
